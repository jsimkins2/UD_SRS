...
# edited the original code here that was intended for aqua and revised it to work for viirs
# involves snapping viirs sst and a_443 data to the aqua lat/lon grid
# James Simkins
require(magick)
...
# read in viirs data
f<- nc_open("http://basin.ceoe.udel.edu/thredds/dodsC/gapfilled_1day_viirs.nc")
#have to read in aqua grid data because that's what we are regridding the viirs data to
a<- nc_open("http://128.175.28.250:8080/thredds/dodsC/Aqua1DayFilled.nc")
alon<-ncvar_get(a, 'lon')
alat<-ncvar_get(a, 'lat')
...
###gapfilled #################
DofY <-yday(cur_tm)
yd<- matrix(DofY, length(alon), length(alat))
ydr<- flip(raster(t(yd)),2)

sstr<- flip(raster(t(fore_sst[,,1])),2)
a_443_qaar<- flip(raster(t(fore_443[,,1])),2)
# upsample the sst and a_443 data to match an Aqua resolution
sstr <- resample(sstr, Depthr)
a_443_qaar <- resample(a_443_qaar, Depthr)
...
ind1<-which(sub_lon == min(alon))
ind2<- which(sub_lon == max(alon))
ind3<- which(sub_lat == min(alat))
ind4<- which(sub_lat == max(alat))
...
## make sure lat and lon are the same as the aqua netcdf because that's what we regridded
# the viirs data to
a<- nc_open("http://128.175.28.250:8080/thredds/dodsC/Aqua1DayFilled.nc")
lon<-ncvar_get(a, 'lon')
lat<-ncvar_get(a, 'lat')
