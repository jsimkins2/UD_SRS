...
for a in range(4,5):
    for h in range(0,168): # 
        ...
        '''
        goes_nc.to_netcdf(path=outpath + 'roffs_' +  'area' + str(a) + '_' + pd.datetime.strftime(pd.Timestamp(goes_nc.time.values[0]),'%Y%j%H%M') + pd.datetime.strftime(gpd.Timestamp(goes_nc.time.values[-1]),'%Y%j%H%M')  + '.nc', format='NETCDF3_CLASSIC')
