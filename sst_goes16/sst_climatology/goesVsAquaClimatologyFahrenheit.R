...
if (!file.exists(paste0("/data/aquaGoesSST/F/", substr(ymdGoes, 1, 4), "/SSTanomalyGoesAqua8dayFahrenheit", ymdGoes, ".nc"))){
  print(ymdGoes)
  ...
  sst = sst - bias
  # convert to celsius so then we can compare to the climatology
  sst = sst - 273.15
  ...
  tmp_def <- ncvar_def("tmp","deg_F",list(londim,latdim,timedim),fillvalue,dlname,prec="single")
  
  var.list <- list()
  var.list[[1]] <- ncdf4::ncvar_def(name="sst", units="Fahrenheit", missval=-999, longname = "Sea Surface Temperature Anomaly", dim=list(londim,latdim,timedim))
  loc.file <- paste0("/data/aquaGoesSST/F/", substr(ymdGoes, 1, 4), "/SSTanomalyGoesAqua8dayFahrenheit", ymdGoes, ".nc")
