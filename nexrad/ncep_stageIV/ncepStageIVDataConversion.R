...
for (d in seq(utc14, utcnow, by="hour")){
  ...
  if (!file.exists(paste0(datadir,'notNetcdf/', substr(temFile, 0, nchar(temFile)-3)))){
    ...
    system(paste0("/usr/local/bin/gdalwarp ",datadir,'unprojected/', temFile,".nc /data/ncep_stageIV/",year(dateTime),"/",temFile, ".nc -of netcdf -t_srs epsg:4326"))
