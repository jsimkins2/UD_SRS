...
#ax.set_extent((min_lon, max_lon, min_lat, max_lat))
#im1 = ax.pcolormesh(glon, glat,rain,cmap=mpl.cm.Greens, transform = ccrs.PlateCarree())
#im2 = ax.pcolormesh(glon, glat,sleet,cmap=mpl.cm.PuRd, transform = ccrs.PlateCarree())
#im3 = ax.pcolormesh(glon, glat,snow,cmap=mpl.cm.Blues, transform = ccrs.PlateCarree())#

#clabeltext='Radial Velocity [m/s]'
#title = 'Thickness Lines Reprojected'
#timestr = local.strftime('%Y-%m-%d %H:%M ') + et
#plt.title(title)
#ax.add_feature(cfeature.NaturalEarthFeature('physical', 'coastline', '10m',
...
#                                edgecolor='black', facecolor='none',linewidth=1))
#ax.add_feature(USCOUNTIES.with_scale('5m'), linewidth=0.5)
#im2 = ax.contour(glon, glat, thick2,levels=[5100,5200,5300,5500,5550,5580, 5640, 5700,5800], colors='k',linestyles='--', transform = ccrs.PlateCarree())
#CS = ax.contour(glon, glat, thick2,levels = [5400],
#                 colors='blue',linestyles='--',linewidths=2, transform = ccrs.PlateCarree())
#plt.clabel(im2, fmt='%1.0f', transform=dat.metpy.cartopy_crs)
#plt.clabel(CS,fmt='%1.0f', transform=dat.metpy.cartopy_crs)
#cbaxes = fig.add_axes([0.9, 0.14, 0.03, 0.25]) 
#cb = plt.colorbar(im1, cax = cbaxes)  

#axt = fig.add_subplot(1,2,2, projection=ccrs.PlateCarree())
#axt.set_extent((min_lon, max_lon, min_lat, max_lat))
#im4 = axt.pcolormesh(glon, glat,rain,cmap=mpl.cm.Greens, transform = ccrs.PlateCarree())
#im2 = axt.pcolormesh(glon, glat,sleet,cmap=mpl.cm.PuRd, transform = ccrs.PlateCarree())
#im3 = axt.pcolormesh(glon, glat,snow,cmap=mpl.cm.Blues, transform = ccrs.PlateCarree())
#
#title = 'Native Thickness Lines'
#timestr = local.strftime('%Y-%m-%d %H:%M ') + et
#plt.title(title)
#axt.add_feature(cfeature.NaturalEarthFeature('physical', 'coastline', '10m',
#                                edgecolor='black', facecolor='none',linewidth=1))
#axt.add_feature(cfeature.NaturalEarthFeature('cultural', 'admin_1_states_provinces_lakes', '50m',
#                                edgecolor='black', facecolor='none',linewidth=1))
#axt.add_feature(cfeature.NaturalEarthFeature('cultural', 'admin_0_boundary_lines_land', '50m',
#                                edgecolor='black', facecolor='none',linewidth=1))
#axt.add_feature(USCOUNTIES.with_scale('5m'), linewidth=0.5)
#im2 = axt.contour(thickness.x.values, thickness.y.values, thickness.values,levels=[5100,5200,5300,5580,5640, 5700,5800], colors='k',linestyles='--', transform = dat.metpy.cartopy_crs)
#CS = axt.contour(thickness.x.values, thickness.y.values, thickness.values,levels = [5400],
#                 colors='blue',linestyles='--',linewidths=2, transform = dat.metpy.cartopy_crs)
#plt.clabel(im2, fmt='%1.0f', transform=dat.metpy.cartopy_crs)
#plt.clabel(CS,fmt='%1.0f', transform=dat.metpy.cartopy_crs)
############
...
ax.plot(lon0, lat0,
         color='k', linewidth=4, marker='o',
         transform=ccrs.PlateCarree())
im1 = ax.pcolormesh(glon, glat,gridsurf,cmap=mpl.cm.jet, vmin=260, vmax=285, transform = ccrs.PlateCarree())
ax.add_feature(cfeature.NaturalEarthFeature('physical', 'coastline', '10m',
                                edgecolor='black', facecolor='none',linewidth=1.5))
ax.add_feature(cfeature.NaturalEarthFeature('cultural', 'admin_1_states_provinces_lakes', '50m',
                                edgecolor='black', facecolor='none',linewidth=1.5))
