...
# James Simkins
# have to run checksite before this script 
import matplotlib
matplotlib.use('agg')
...
ABI_datetime = []
for i in match:    
    if os.path.isfile("/home/sat_ops/goes_r/lightning/add_radar/" + image_dir + str(i) + ".png") == False:
...
GLM_int = []
ABI_int = []
for st in xrange(0, len(abi_match)):
    x = abi_match[st]
    ABI_int.append(int(str(ABI_datetime[x]) + '000'))
...
# likewise, we don't want to run if abi_match is of length 0    
if len(abi_match) > 0:
    ...
    for i in xrange(0, len(abi_match)):
        ...
        if time_diff[0] < 11:
            ...
            if os.path.isfile(C_file) == False:
                C_file = '/home/sat_ops/goes_r/cloud_prod/noaa_format/data/OR_ABI-L2-CMIPC-M4C02_G16_s' + str(ABI_datetime[abi]) + '.nc'
            ...
            ltng_lat = {}
            ltng_lon = {}
            for lt in xrange(0, len(abi_glm[i])):
                ltfile = abi_glm[i][lt]
            ...
            # adding this as a buffer, trying to clean up the low radar returns 6/18/2018
            dBZ[dBZ < 12] = np.ma.masked
            ...
            for g in xrange(len(ltng_lat) - 3, len(ltng_lat)):
                group_x, group_y = mH(ltng_lon[g], ltng_lat[g])
                mH.scatter(group_x, group_y, s=10, marker="D", c='white', edgecolor='black', lw=1.2, ax=ax)
            ...
            #fig.text(0.5,0.95, site + ' (0.5$^{\circ}$) Reflectivity ' + 
                    ...
                    # should be .94 below
            fig.text(0.5,0.94, 'NOAA GOES-16 & ' + site + ' Radar Reflectivity & Lightning Flashes \n' +
            ...
            # save file
            output_file = '/home/sat_ops/goes_r/lightning/add_radar/' + image_dir + str(ABI_datetime[abi]) + ".png"
