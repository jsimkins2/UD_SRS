...
chester_ds = xr.open_dataset("/data/DEOS/chester/chester_agwx.nc")
ncc_ds = xr.open_dataset("/data/DEOS/ncc/ncc_agwx.nc")
kent_ds = xr.open_dataset("/data/DEOS/kent/kent_agwx.nc")
sussex_ds = xr.open_dataset("/data/DEOS/sussex/sussex_agwx.nc")
...
for co in range(0, len(deos_boundarys["NAME"])):
    ...
    var_list = []
    for t in range(0,len(chester_ds.time.values)):
        da = xr.DataArray(dsPrec.Precipitation_Flux.sel(time = chester_ds.time.values[t], method = 'nearest').values,dims=['latitude', 'longitude'],coords={'longitude': agwx_main.longitude.values, 'latitude' :agwx_main.latitude.values})
...
chester_ds.to_netcdf("/data/DEOS/ncep/chester_agwx.nc")
print('finished chester county')

ncc_ds.to_netcdf("/data/DEOS/ncep/ncc_agwx.nc")
print('finished new castle county')

kent_ds.to_netcdf("/data/DEOS/ncep/kent_agwx.nc")
print('finished kent county')

sussex_ds.to_netcdf("/data/DEOS/ncep/sussex_agwx.nc")
